...
based on higher order logic (``HOL''), the usual starting point to import is the theory @{theory Main}.
...
The Isabelle distribution provides heap files for the session \<^verbatim>\<open>HOL/HOL\<close> and its parent session \<^verbatim>\<open>Pure/Pure\<close>,
...
The editor also loads a session (together with its ancestors), the default session to load is HOL.
...
The user interface consists of a text area which is surrounded by docking areas where
additional panels can be displayed. Several panels can be displayed in the same docking area,
...
text\<open>
Most identifiers used in Isabelle content have been defined in some theory file, this also holds
...
text\<open>
The Sidekick panel displays a structured view of the content of the text area and supports
...
text\<open>
The Output panel displays the result of the theory text processing when it reaches the 
...
text\<open>
The State panel displays a specific result of the theory text processing
if the cursor position is in a proof. It is called the ``goal state'' (see 
Section~\ref{basic-proof-state}), and describes what remains to be proved by the
rest of the proof.
...
are usually not on the keyboard. The Symbols panel can be used to input such
...
of keys, called an abbreviation. As an example, the sequence \<^verbatim>\<open>==>\<close> is an abbreviation for the
...
through the Documentation panel. This manual refers to some of these documentations,
if applicable. 

For example, more information about the use of the interactive editor can be found 
in the Isabelle Reference Manual about jedit.
\<close>
...
text\<open>
The Query panel supports active searching for items in the content of all loaded sessions. There are
several tabs for searching different kinds of items. Depending on the kind, a search specification
...
search specifications for different kinds of items can be found in Chapter~\ref{basic}.
\<close>
...
text\<open>
The Theories panel displays the loaded session and the opened or imported theories which do
